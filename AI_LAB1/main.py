        ...
        if firstFit == 1:
            ...
            for i in itemsStr.split():
                if i != '\n':
                    items.append(int(i))
            c = int(input("ENTER BIN CAPACITY\n"))
            binpacking = BinPacking(c, GA_POPSIZE, GA_ELITRATE, GA_MUTATION, maxNum, items)
            start = time.time()
            binpacking.start()
            end = time.time()
            # total time taken
...
if __name__ == '__main__':
    ...
    while run:
        ...
        if problem == 4:
            run = False
    """
    x = PSO(GA_POPSIZE, GA_TARGET, GA_ELITRATE, GA_MUTATIONRATE, GA_MUTATION, GA_MAXITER)
    x.run()
    ...
    minimalconf= MinimalConflict(12)
    minimalconf.start()
    ---------------------------------------------
